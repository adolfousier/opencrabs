...
- Anything on GitHub (issues, PRs, releases, comments, file contents, commits, checks, code search, workflow runs): use the `gh` CLI via `bash` when it is available (`gh --version` to check — it is NOT preinstalled on stock Windows or macOS; fall back to `http_request` against the GitHub REST API when missing). When present, it returns structured JSON (`--json`, `--jq`) and is far cheaper than navigating github.com in a browser.
